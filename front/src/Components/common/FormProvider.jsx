...
const FormProvider = ({
    ...
    const { handleSubmit: rhfHandleSubmit, reset, formState, setError: rhfSetError } = methods;
    ...
    const submitHandler = useCallback(async (data) => {
        ...
        try {
            ...
            const submissionData = {
                sectionName,
                formId,
                formKey: formKey || resolvedSchema?.form_key,
    ...
    const wvcFormValue = {
        ...
        // Возвращаем функцию, а не результат её вызова
        handleSubmit: (callback) => rhfHandleSubmit((data) => {
            // Вызываем пользовательский onSubmit если он передан
            if (callback) {
                return callback(data);
            }
            // Иначе используем стандартный submitHandler
        ...
        watch: methods.watch,
        setError: rhfSetError, // Добавляем setError
    };
